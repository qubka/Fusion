...
namespace ImGui
{
    ...
    public:
        ...
        // set labels for the control panel
        void SetLabels(const std::string& folderLabel, const std::string& fileLabel, const std::string& newFolderLabel, const std::string& refreshFolderLabel);
        ...
        // get the file browser flags
        ImGuiFileBrowserFlags GetFlags() const noexcept;

        // set the file browser flags
    ...
    private:
        ...
        std::string refreshFolderLabel_ = "*";
        std::string newFolderLabel_ = "+";
        std::string directoryLabel_ = "[D]";
        std::string fileLabel_ = "[F]";
...
inline void ImGui::FileBrowser::Display()
{
    ...
    if(SmallButton(refreshFolderLabel_.c_str()))
    {
        UpdateFileRecords();
    ...
    {
        SameLine();
        if(SmallButton(newFolderLabel_.c_str()))
        {
...
inline void ImGui::FileBrowser::SetLabels(const std::string& folderLabel, const std::string& fileLabel, const std::string& newFolderLabel, const std::string& refreshFolderLabel)
{
    fileLabel_ = fileLabel;
    directoryLabel_ = folderLabel;
    newFolderLabel_ = newFolderLabel;
    refreshFolderLabel_ = refreshFolderLabel;
}
...
inline ImGuiFileBrowserFlags ImGui::FileBrowser::GetFlags() const noexcept
{
    return flags_;
}
...
{
    fileRecords_ = { FileRecord{ true, "..", directoryLabel_ + " ..", "" } };
    ...
    for(auto &p : std::filesystem::directory_iterator(pwd_))
    {
        ...
        rcd.showName = (rcd.isDir ? directoryLabel_ : fileLabel_) + " " +
