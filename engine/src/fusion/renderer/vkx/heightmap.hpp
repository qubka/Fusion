...
namespace vkx {
    ...
    public:
        ...
        float getHeight(uint32_t x, uint32_t y) {
            glm::ivec2 s{static_cast<int>(scale)};
            glm::ivec2 rpos{ glm::ivec2{x, y} * s };
        ...
        void fromFile(const vkx::Context& context, const std::string& filename, uint32_t patchsize, const glm::vec3& scale, Topology topology) {
            ...
            for (uint32_t x = 0; x < patchsize; x++) {
                for (uint32_t y = 0; y < patchsize; y++) {
                    ...
                    vertices[index].pos[0] = (x * wx + wx / 2.0f - static_cast<float>(patchsize) * wx / 2.0f) * scale.x;
                    vertices[index].pos[1] = -getHeight(x, y);
                    vertices[index].pos[2] = (y * wy + wy / 2.0f - static_cast<float>(patchsize) * wy / 2.0f) * scale.z;
                    vertices[index].uv = glm::vec2{static_cast<float>(x) / patchsize, static_cast<float>(y) / patchsize} * uvScale;
                }
            }
            ...
            for (uint32_t y = 0; y < patchsize; y++) {
                for (uint32_t x = 0; x < patchsize; x++) {
                    ...
                    glm::vec3 A{1.0f, 0.0f, dx};
                    glm::vec3 B{0.0f, 1.0f, dy};
