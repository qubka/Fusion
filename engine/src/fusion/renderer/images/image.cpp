...
#include "fusion/renderer/commands/command_buffer.hpp"

using namespace fe;

constexpr static float ANISOTROPY = 16.0f;

Image::Image(VkFilter filter, VkSamplerAddressMode addressMode, VkSampleCountFlagBits samples, VkImageLayout layout, VkImageUsageFlags usage, VkFormat format, uint32_t mipLevels, uint32_t arrayLayers, const VkExtent3D& extent)
    : extent{extent}
    , samples{samples}
    , usage{usage}
    , format{format}
...
std::unique_ptr<Bitmap> Image::getBitmap(uint32_t mipLevel, uint32_t arrayLayer) const {
	...
	glm::uvec2 size{ uint32_t(extent.width >> mipLevel), uint32_t(extent.height >> mipLevel) };
