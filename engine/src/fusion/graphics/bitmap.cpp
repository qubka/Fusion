    ...
    if (fileExt == ".pic" || fileExt == ".hdr") {
        File::Read(filename, [&](size_t size, const void* data) {
            pixels = std::unique_ptr<uint8_t[]>(
                    reinterpret_cast<uint8_t*>(stbi_loadf_from_memory(reinterpret_cast<const uint8_t*>(data),
        ...
        File::Read(filename, [&](size_t size, const void* data) {
            pixels = std::unique_ptr<uint8_t[]>(
                    stbi_load_from_memory(reinterpret_cast<const uint8_t*>(data), static_cast<int>(size), &width,
