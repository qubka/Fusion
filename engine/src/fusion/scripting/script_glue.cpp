...
static void NativeLog(MonoString* string, int parameter) {
    ...
    FE_LOG_WARNING("{}, {}", str, parameter);
}

static void NativeLog_Vector(glm::vec3* parameter, glm::vec3* outResult) {
    FE_LOG_WARNING("Value: {}", *parameter);
...
static bool Entity_HasComponent(uint32_t entityID, MonoReflectionType* componentType) {
    ...
    FE_ASSERT(it != EntityHasComponentFuncs.end());
...
static void RegisterComponent() {
    ([]() {
        ...
        if (!managedType) {
            FE_LOG_ERROR("Could not find component type: {}", managedTypename);
            return;
        }
