#include "parent_system.hpp"
#include "fusion/scene/components.hpp"

using namespace fe;

bool ParentSystem::has_children(const entt::entity entity) const {
    if (auto component = registry.try_get<RelationshipComponent>(entity); component && component->children > 0) {
        return true;
    }
    return false;
}
...
void ParentSystem::remove_parent(const entt::entity entity) {
    if (auto component = registry.try_get<RelationshipComponent>(entity); component && component->children > 0) {
        ...
        while (curr != entt::null) {
            auto& i = registry.get<RelationshipComponent>(curr);
            auto next = i.next;
            if (i.children == 0) {
                registry.erase<RelationshipComponent>(curr);
            } else {
                i.prev = entt::null;
                i.next = entt::null;
                i.parent = entt::null;
        ...
        registry.erase<RelationshipComponent>(entity);
    }
}
...
void ParentSystem::remove_child(const entt::entity parent, const entt::entity child) {
    if (auto component = registry.try_get<RelationshipComponent>(parent); component && component->children > 0) {
        if (component->children == 1) {
            registry.get<RelationshipComponent>(child).parent = entt::null;
            ...
            while (curr != entt::null) {
                ...
                if (curr == child) {
                    if (i.prev != entt::null) {
                        registry.get<RelationshipComponent>(i.prev).next = i.next;
                    } else {
                    ...
                    if (i.next != entt::null) {
                        registry.get<RelationshipComponent>(i.next).prev = i.prev;
                    }
                    if (i.children == 0) {
                        registry.erase<RelationshipComponent>(curr);
                    } else {
        ...
        if (component->children == 0 && component->prev == entt::null && component->next == entt::null && component->parent == entt::null) {
            registry.erase<RelationshipComponent>(parent);
        }
    }
}
